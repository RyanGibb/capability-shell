  ...
  @@ object
       ...
       method shell_impl params release_param_caps =
         ...
         Eio.Fiber.fork ~sw (fun () ->
             let e =
               Eio.Promise.await (Eio_linux.Low_level.Process.exit_status shell)
             in
       ...
       method mosh_impl _ release_param_caps =
         ...
         | Some mosh_addr ->
             ...
             Api.Builder.MoshConnect.ip_set mosh_connect
               (Ipaddr.to_string mosh_addr);
...
type mosh_connect = { ip : Ipaddr.t; port : int32; key : string }
